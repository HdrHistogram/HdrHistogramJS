...
describe("Histogram Log Writer", () => {
  ...
  it("should write a line with start time, duration, max value, and a base64 encoded histogram", async () => {
    // given
    histogram.recordValue(123000);
    // when
    await writer.outputIntervalHistogram(histogram, 1000, 1042);
    // then
    expect(buffer).toMatch(/^1000.000,42.000,123.000,HISTFAA/);
  });

  it("should write start time, duration and  max value using 3 digits", async () => {
    // given
    histogram.recordValue(123001);
    // when
    await writer.outputIntervalHistogram(histogram, 1000.0120001, 1042.013001);
    // then
    expect(buffer).toMatch(/^1000.012,42.001,123.001,HISTFAA/);
  });

  it("should write a line starting with histogram tag", async () => {
    // given
    histogram.tag = "TAG";
    histogram.recordValue(123000);
    // when
  ...
  it("should write a histogram's start time in sec using basetime", async () => {
    // given
