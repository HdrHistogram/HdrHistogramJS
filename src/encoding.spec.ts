/*
 * This is a TypeScript port of the original Java version, which was written by
 * Gil Tene as described in
 * https://github.com/HdrHistogram/HdrHistogram
 * and released to the public domain, as explained at
 * http://creativecommons.org/publicdomain/zero/1.0/
 */
import { Int32Histogram, WasmHistogram } from "./index";
import { initWebAssembly } from "./wasm";
import {
  encodeIntoCompressedBase64,
  decodeFromCompressedBase64,
} from "./encoding";
import Histogram from "./Histogram";

describe("Histogram encoding", () => {
  it("should encode and decode an histogram", async () => {
    // given
    const histogram = new Int32Histogram(1, 2, 3);
    histogram.autoResize = true;
    histogram.recordValue(32415);
    // when
    const b64 = await encodeIntoCompressedBase64(histogram);
    // then
    const decodedHistogram = await decodeFromCompressedBase64(b64);
    expect(decodedHistogram.toJSON()).toStrictEqual(histogram.toJSON());
  });

  it("should decode and decompress reading a base64 string", async () => {
    // given
    const base64String = "HISTFAAAAB542pNpmSzMwMDAxAABzFCaEUoz2X+AMIKZAEARAtM=";
    // when
    const histogram = await decodeFromCompressedBase64(base64String);
    // then
    expect(histogram.mean).toBe(42);
    expect(histogram.totalCount).toBe(1);
  });

  it("should decode and decompress a big base64 string", async () => {
    // given
    const base64String =
      "";
    // when
    const histogram = await decodeFromCompressedBase64(base64String);
    // then
    expect(histogram.totalCount).toBe(10000);
  });

  it("should encode and compress an histogram to a base64 string that roundtrips", async () => {
    // given
    const histogram = new Int32Histogram(1, 2, 3);
    histogram.recordValue(42);
    // when
    const base64Histogram = await encodeIntoCompressedBase64(histogram);
    // then — the exact bytes depend on the deflate implementation, so assert a
    // successful round-trip rather than literal bytes
    expect(base64Histogram.startsWith("HIST")).toBe(true);
    const decodedHistogram = await decodeFromCompressedBase64(base64Histogram);
    expect(decodedHistogram.mean).toBe(42);
    expect(decodedHistogram.totalCount).toBe(1);
  });

  it("should throw an error when trying to decompress an histogram using V1 encoding", async () => {
    // given
    const base64V1EncodingString =
      "HISTIgAAAFd42pNpmazIwMAYxgABTBDKT4GBgdnNYMcCBvsPUBkeBkYGZqA8MwMbAzsDC5DFBCTZgJCDQY1BjkGLQZRBlUEPCB8zWDCYMxgDZZkZhgJgHDibAY8JB/A=";
    // when & then
    await expect(
      decodeFromCompressedBase64(base64V1EncodingString)
    ).rejects.toThrowError("Encoding not supported");
  });

  it("should reject without leaking an unhandled rejection on corrupt compressed data", async () => {
    // given: a valid V2 compressed cookie (0x1c849314) + length 4 + an invalid
    // zlib payload (00 00 00 00). This passes the cookie check and reaches the
    // native DecompressionStream, which errors on the bad zlib header.
    const corruptBase64 = "HISTFAAAAAQAAAAA";

    // capture unhandled rejections ourselves (and isolate from the test runner's
    // own handler) so the assertion is the sole, deterministic signal
    const previousListeners = process.listeners("unhandledRejection");
    previousListeners.forEach((l) =>
      process.off("unhandledRejection", l as any)
    );
    const leaked: unknown[] = [];
    const capture = (reason: unknown) => leaked.push(reason);
    process.on("unhandledRejection", capture);

    try {
      // when & then: the call rejects as intended...
      await expect(decodeFromCompressedBase64(corruptBase64)).rejects.toThrow();
      // ...and no extra rejection escapes from the compression stream's writer side
      await new Promise((resolve) => setTimeout(resolve, 50));
      expect(leaked).toEqual([]);
    } finally {
      process.off("unhandledRejection", capture);
      previousListeners.forEach((l) =>
        process.on("unhandledRejection", l as any)
      );
    }
  });
});

describe("WASM Histogram encoding", () => {
  let histogram: Histogram;

  beforeAll(initWebAssembly);

  afterEach(() => {
    if (histogram) {
      histogram.destroy();
    }
  });

  it("should decode and decompress a big base64 string", async () => {
    // given
    const base64String =
      "";
    // when
    histogram = await decodeFromCompressedBase64(base64String, 32, true);
    // then
    //console.log(histogram);
    expect(histogram.totalCount).toBe(10000);
  });

  it("should decode and decompress a big base64 string to a wasm packed histogram", async () => {
    // given
    const base64String =
      "";
    // when
    histogram = await decodeFromCompressedBase64(base64String, "packed", true);
    // then
    expect(histogram.totalCount).toBe(10000);
  });

  it("should encode and decode an histogram", async () => {
    // given
    histogram = WasmHistogram.build();
    histogram.recordValue(32415482);
    // when
    const b64 = await encodeIntoCompressedBase64(histogram);
    // then
    const decodedHistogram = await decodeFromCompressedBase64(b64);
    expect(decodedHistogram.getValueAtPercentile(50)).toBe(
      histogram.getValueAtPercentile(50)
    );
  });
});
